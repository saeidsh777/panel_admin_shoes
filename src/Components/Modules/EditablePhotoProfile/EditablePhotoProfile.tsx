...
export default function EditablePhotoProfile() {
    const [isImage, setIsImage] = useState('');
    const [defaultImg, setDefaultImg] = useState(
        '/images/user/user-default-men.svg'
    );
    const [image, setImage] = useState<File | null>(null);
    const [strigInit, setStrigInit] = useState('');
    const [isChange, setIsChange] = useState(false);

    useEffect(() => {
        setStrigInit(isImage + '/images/user/user-default-men.svg');
    }, []);

    useEffect(() => {
        const stringContact = isImage + defaultImg;
        const isEqual = stringContact === strigInit;
        if (isEqual) {
            setIsChange(false);
        } else {
            setIsChange(true);
    ...
    const removeImg = () => {
        setIsImage('');
        setDefaultImg('/images/user/user-default-men.svg');
        setImage(null);
    };

    return (
        <div>
            <div className="w-24 group relative mx-auto mb-6">
                {defaultImg && !isImage && (
            ...
            {isChange && (
                <button className="btn_global mx-auto block">Save</button>
            )}
        </div>
    );
}
